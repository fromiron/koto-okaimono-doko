...
export type ValidationReport = {
  ...
  duplicateIds: string[];
  invalidOfficialDetailUrls: string[];
...
export function validateStores(stores: Store[]): ValidationReport {
  ...
  return {
    ...
    invalidOfficialDetailUrls: stores
      .filter(
        (store) =>
          store.officialDetailUrl !== null && !isAllowedOfficialDetailUrl(store.officialDetailUrl),
      )
      .map((store) => store.id),
...
export function assertValidForBuild(report: ValidationReport): void {
  ...
  if (report.invalidOfficialDetailUrls.length > 0) {
    errors.push(`${report.invalidOfficialDetailUrls.length} invalid official detail URLs found.`);
  }
